...
class EyeTracker:
    ...
    def calculate_gaze_direction(self, landmarks, h, w):
        ...
        for iris_indices, eye_corners in [(self.LEFT_IRIS, self.LEFT_EYE_CORNERS),
            ...
            inner_corner = np.array([landmarks[eye_corners[0]].x * w, landmarks[eye_corners[0]].y * h])
            outer_corner = np.array([landmarks[eye_corners[1]].x * w, landmarks[eye_corners[1]].y * h])
    ...
    def draw_eye_parts(self, img, landmarks):
        ...
        if all(idx < len(landmarks) for idx in self.LEFT_IRIS):
            ...
            cv2.circle(overlay, tuple(left_iris_center), left_iris_radius, (255, 100, 0), -1)
            left_pupil_center, left_pupil_radius = self.detect_pupil(img, left_iris_center, left_iris_radius)
            cv2.circle(overlay, tuple(left_pupil_center), left_pupil_radius, (0, 0, 0), -1)
            eye_points = []
        ...
        if all(idx < len(landmarks) for idx in self.RIGHT_IRIS):
            ...
            cv2.circle(overlay, tuple(right_iris_center), right_iris_radius, (255, 100, 0), -1)
            right_pupil_center, right_pupil_radius = self.detect_pupil(img, right_iris_center, right_iris_radius)
            cv2.circle(overlay, tuple(right_pupil_center), right_pupil_radius, (0, 0, 0), -1)
            eye_points = []
        ...
        cv2.putText(img, f"EAR: {ear_value:.3f}", (10, y_offset),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 255), 2)
        y_offset += 25
        cv2.putText(img, f"UDP: Sending {gaze_text}", (10, y_offset),
    ...
    def run(self):
        ...
        while cap.isOpened():
            ...
            if results.multi_face_landmarks:
                ...
                frame, gaze_text = self.draw_eye_parts(frame, face_landmarks.landmark)
                try:
                    sock.sendto(gaze_text.encode('utf-8'), serverAddressPort)
